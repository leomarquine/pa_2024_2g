...
async function main() {
    ...
    for (let index = 2; index <= tab.rowCount; index++) {
        ...
        for (const [materia, coluna] of Object.entries(colunas)) {
            const etapas = JSON.parse(row.getCell(coluna).text.replace(/'/g, '"').replace(/None/g, 'null'));
...
async function carregaTurmas(tab) {
    const nomes = new Set();
